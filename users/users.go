...
func LoadUserInfo(title string, r *http.Request) (filename string, option []byte, usr []byte, bar []byte) {
	...
	if cookies.IsLoggedIn(r) {
		...
		str := "Commit Collection: <br><br>"
		// TODO instead of downloading it may make more sense to bring
		// them to a page where they can view the commits "info"
		for i := 0; i < 5; i++ {
			if  i < len(userProfile.CommitList) {
				commit := commits.FindCommit(userProfile.CommitList[i])
				link := "<a href='commits/" + commit.VersionID + ".info'>"
				str +=  link + commit.Title + "</a><br><br>"
			} else {
				str += "<a href='/add-commit.txt'>Add Commit!</a><br><br>"
			}
		}
...
func CreateUserFile(usrName string) {
	...
	s += "If you would like to add a commit: "
	s += "<a href='/add-commit'>click here!</a>"
	// TODO add a plugin to a database of commits
	file.WriteString(s)
...
/** Checks if a commit is in the users personal commitlist
 * If the commit was already in the collection, then false will be returned.
**/
func UpdateCollection(email string, commit *commits.Commit) bool {
	...
	if userProfile == nil {
		...
		for i := 0; i < len(userProfile.CommitList); i++ {
			if userProfile.CommitList[i] == id {
				return false
			}
		}
	}
	userProfile.CommitList = append(userProfile.CommitList, (*commit).Id)
